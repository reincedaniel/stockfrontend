        ...
        <q-tab-panel name="itens">
          <div class="q-pa-md">
            <div class="row q-gutter-sm">
              ...
              <q-btn size="md" label="Exportar - Excel" color="positive" @click="onexport" icon="print" />
            </div>
        </div>
          <div class="q-pa-md">
            <!-- TABLE Articles -->
          ...
          for (let i in this.data) {
            if (this.data.hasOwnProperty(i)) {
                    this.Datas.animals.push({
                          id: this.data[i].id,
                          Código: this.serializeUs(this.data[i].internal_code),
                         Artigo: this.data[i].description,
                         Fabricante: this.serializeUs(this.data[i].manufactured_by), 
                          Modelo: this.serializeUs(this.data[i].modelo),
                          Fornecedor:
                            this.data[i].Provider == null
                              ? "Nenhum"
                              : this.data[i].Provider,
                          Preço : this.data[i].purchase == null
                              ? "0"
                              : this.serializeUs(this.data[i].purchase),

                          IVA : this.data[i].iva == null
                              ? "0%"
                              : this.serializeUs(this.data[i].iva)+"%",
                          Desconto : this.data[i].discount == null
                              ? "0"
                              : this.serializeUs(this.listInputs[i].discount),
                          Total : this.data[i].total == null
                              ? "0"
                              : this.serializeUs(this.data[i].total),
                          Quantidade: this.serializeUs(this.data[i].quantity_input),
                          Data: this.data[i].createdAt
                        });
                }
          }    
